...
struct OneEuroFilter {
  ...
  // Aggressive snappy responsiveness (2.0Hz cutoff, 0.05 beta)
  OneEuroFilter() : min_cutoff(2.0f), beta(0.05f), d_cutoff(1.0f) {}
...
static void gst_mozza_mp_gpu_class_init(GstMozzaMpGpuClass* klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_float("min-cutoff", "Min cutoff frequency",
                         "OneEuroFilter min_cutoff: lower = more smoothing at rest (less jitter, more lag). Default 2.0 Hz.",
                         0.001f, 100.0f, 2.0f, G_PARAM_READWRITE));
  g_object_class_install_property(
